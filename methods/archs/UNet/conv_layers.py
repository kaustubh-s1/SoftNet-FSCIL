...
class SEBlock(nn.Module):
    def __init__(self, in_ch, ratio=4, act=nn.ReLU):
        ...
        self.excitation = nn.Sequential(
                        SignetConv3d(in_ch, in_ch//ratio, kernel_size=1),
                        act(),
                        SignetConv3d(in_ch//ratio, in_ch, kernel_size=1),
                        nn.Sigmoid()
        )
    def forward(self, x):
        out = self.squeeze(x)
